...
-(NSDictionary*) loadPropertiesFile {
    ...
    plistPath = [rootPath stringByAppendingPathComponent:@"serverReactor.plist"];
    if (![[NSFileManager defaultManager] fileExistsAtPath:plistPath]) {
        plistPath = [[NSBundle mainBundle] pathForResource:@"serverReactor" ofType:@"plist"];
    }
...
-(Boolean)createHandler: (int) i fromDictionary: (NSDictionary*) dict{
    ...
    NSLog(@"creating handler");
    
    if (dict == nil){
        NSLog(@"dictionary was nil");
        return false;
    }
...
-(void) dispatch:(Event *)event {
    NSLog(@"Dispatching event handler for type %@", event.type);
...
-(void) register:(id)eventHandler with: (SEL) s forType:(NSString *)type {
       @synchronized(handlers) {
        if ([eventHandler respondsToSelector:s]) {
            ...
            NSLog(@"adding to handlers dictionary");
            [handlers setObject: instance forKey: type];
        }
        else{
            NSLog(@"did not respond to selector");
